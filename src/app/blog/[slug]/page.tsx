...
export async function generateMetadata({
  ...
  if (!post) return {};
  const title = (post.data.title as string) ?? "Blog";
  const description = (post.data.excerpt as string) ?? undefined;
  return {
    title,
    description,
    openGraph: {
      title: `${title} | Sweet Desert`,
      description,
      type: "article",
    },
  };
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  return (
    <>
      <JsonLd data={breadcrumbSchema(title, `/blog/${slug}`)} />
